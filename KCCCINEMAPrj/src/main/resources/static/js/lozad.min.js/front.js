...
mbAlert = (function() {
	...
	return {
		...
		update: function() {
			...
			if (document.querySelector('.layer-con').offsetHeight > 600) important = " !important";
			...
			$(tgtID)
				.css({
					top: th / 2 - $(tgtID).outerHeight(true) / 2,
				...
				switch (cs) {
					case 'open':
						...
						if (x <= w / 2) {
							...
							dS.css.set({
								target: dom.tooltipPin,
								property: {
									top: Math.floor(pt) + 'px', left: Math.floor(v - 15) + 'px',
							...
							dS.css.set({
								target: dom.tooltipPin,
								property: {
									top: Math.round(z - 10) + 'px', left: v + (w / 2) - 11 + 'px',
							...
							dS.css.set({
								target: dom.tooltipPin,
								property: {
									top: Math.round(z + h) + 'px', left: Math.round(v + (w / 2) - 11) + 'px',
		...
		, events: {
			...
			, move: {
				day: function(d) {
					...
					TweenLite.to(dom.dateM, .3, {
						left: (d === 'next') ? -(setting.dwidth * 2) : 0, ease: Power1.easeOutCubic,
		...
		, events: {
			...
			, setArrange: function() {
				...
				if (dom.talsE.getAttribute('reLoced') === null) {
					...
					cell.c = {
						x: String.prototype.getMatrix.call(getComputedStyle(dom.talsE)[setting.trsfm], 4),
...
window.onload = function() {
	...
	if (typeof mbThCalendar === 'object') mbThCalendar.init({
		target: 'date-area',
		fetchHoliday: setHldyAdopt, fetchDisday: setDisdyAdopt,
		holidays: 'holidaysFromServer', disdays: 'disdaysFromServer'
	}); // 상영관 달력
